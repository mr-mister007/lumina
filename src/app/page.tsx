...
export default function Lumina() {
  ...
  const researchTopic = async () => {
    ...
    try {
      setResearchStep("Starting autonomous research engine...");
      ...
      // Auto-trigger synthesis after research
      setResearchStep("Synthesizing multi-source findings...");
      await analyze(data.sources);
  ...
  const analyze = async (providedSources?: SourceInput[]) => {
    const targetSources = providedSources || sources;
    if (targetSources.some(s => !s.content.trim())) {
    ...
    try {
      ...
      const nodes: Node[] = [
        ...
        ...claimNodes.map((n: any, idx: number) => ({
          ...
          type: 'default',
          data: { label: n.label },
          position: { x: 600 + (idx % 2 === 0 ? 0 : 300), y: idx * 150 },
      ...
      <nav className="border-b border-neutral-900 bg-[#0a0a0b]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs font-medium uppercase tracking-widest text-neutral-500">
            <span>Cognitive Load Balancer v2.0</span>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto p-6 pt-12">
        <header className="mb-12 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-b from-white to-neutral-400 bg-clip-text text-transparent">
            Autonomous Truth Synthesis
          </h2>
          <p className="text-lg text-neutral-400 max-w-2xl mx-auto">
            Deep research engine powered by an agentic loop and real-time synthesis.
          </p>
        </header>
        ...
        {!result && (
          <div className="max-w-3xl mx-auto mb-16">
            <div className="relative group">
              ...
              <div className="relative flex items-center bg-neutral-900/90 border border-neutral-800 p-2 rounded-full backdrop-blur-xl shadow-2xl">
                ...
                <input 
                  ...
                  placeholder="Enter a topic for deep research..."
            ...
            {loading && researchStep && (
              <motion.div 
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mt-8 p-8 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/20 flex flex-col items-center gap-6 shadow-2xl"
              >
                <div className="flex items-center gap-3 text-emerald-400 font-bold text-lg animate-pulse">
                  <Brain className="w-6 h-6" />
                  {researchStep}
                </div>
                <div className="w-full max-w-md bg-neutral-800 h-2 rounded-full overflow-hidden shadow-inner">
                  <motion.div 
                ...
                <p className="text-neutral-500 text-sm font-medium tracking-wide">GEMINI 2.5 FLASH IS AGENTICALLY BROWSING...</p>
              </motion.div>
            )}
          </div>
        )}
        ...
        {!result ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
            <AnimatePresence>
              {sources.map((source, index) => (
                <motion.div
                  ...
                  className="bg-neutral-900/40 border border-neutral-800 p-4 rounded-3xl group transition-all hover:border-emerald-500/30 shadow-lg"
                >
                  ...
                  <textarea
                    ...
                    className="w-full h-48 bg-neutral-900/60 border border-neutral-800 rounded-2xl p-4 text-sm resize-none focus:outline-none focus:ring-1 focus:ring-emerald-500/50 transition-all placeholder:text-neutral-600 shadow-inner"
                  />
                </motion.div>
              ))}
            </AnimatePresence>
            ...
            <button
              onClick={addSource}
              className="h-full min-h-[150px] border-2 border-dashed border-neutral-800 rounded-3xl flex flex-col items-center justify-center gap-3 text-neutral-500 hover:border-emerald-500/50 hover:text-emerald-400 transition-all group hover:bg-emerald-500/5 shadow-sm"
            >
            ...
            <div className="md:col-span-2 lg:col-span-3 flex flex-col items-center gap-4 mt-8">
              {error && (
                <div className="flex items-center gap-2 text-red-400 bg-red-400/10 px-4 py-2 rounded-full text-sm border border-red-400/20 shadow-lg">
              ...
              >
                ...
                {loading ? "Processing..." : "Synthesize Knowledge"}
              </button>
            </div>
          </div>
        ) : (
          <div className="space-y-12 pb-24">
                  ...
                  <div className="space-y-6">
                    {result.tensionPoints.map((tp: any, i: number) => (
                      <motion.div 
                        ...
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="p-6 rounded-3xl bg-amber-500/5 border border-amber-500/20 space-y-4 shadow-sm"
                      >
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-neutral-300 italic leading-relaxed">"{tp.claimA.text}"</p>
                          </div>
                          <div className="space-y-2 border-l border-amber-500/10 pl-4">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 block">Source {tp.claimB.sourceId}</span>
                            <p className="text-xs text-neutral-300 italic leading-relaxed">"{tp.claimB.text}"</p>
                          </div>
                        </div>
                        <div className="pt-4 border-t border-amber-500/10">
                          <p className="text-sm text-amber-200/80 font-medium leading-relaxed">{tp.explanation}</p>
                        </div>
                      </motion.div>
                    ))}
                  </div>
            ...
            <section className="bg-neutral-900/40 border border-neutral-800 rounded-[2.5rem] h-[750px] shadow-2xl overflow-hidden relative group">
              <div className="absolute top-6 left-8 z-10">
                ...
                <p className="text-xs text-neutral-500 mt-1">Relationship Map: Sources ➔ Synthesized Intelligence</p>
              </div>
              <ReactFlow
                nodes={result.nodes}
                edges={result.edges}
            ...
            <div className="flex justify-center pb-12">
              <button 
                ...
                className="text-neutral-500 hover:text-white transition-all flex items-center gap-2 bg-neutral-900/40 hover:bg-neutral-900/60 px-8 py-3 rounded-full border border-neutral-800 hover:border-neutral-700 shadow-sm"
              >
                <RefreshCcw className="w-4 h-4" /> Start New Research
              </button>
            </div>
          </div>
        )}
